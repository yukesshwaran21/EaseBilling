...
export default function AdminDashboard() {
  ...
  // 2. Add the jsPDF function after the deleteWorker function
  const generateTransactionPDF = () => {
    ...
    doc.text("Company: Your Company Name", 14, 35);
    doc.text("Contact: your-email@example.com", 14, 42);
    ...
    // Table header
    const headers = ["Bill ID", "Customer", "Date", "Amount"];
    const columnWidths = [40, 60, 40, 30];
    let y = 55;
    ...
    invoices.forEach((invoice, index) => {
      ...
      x = 14;
      doc.text(invoice.billId, x, y);
      x += columnWidths[0];
      doc.text(invoice.customerName, x, y);
      x += columnWidths[1];
      doc.text(new Date(invoice.date).toLocaleDateString(), x, y);
      x += columnWidths[2];
      doc.text(`₹${invoice.totalBill}`, x, y);
    ...
    doc.setFont(undefined, "bold");
    doc.setFontSize(10);
    doc.text(`Total Transactions: ${invoices.length}`, 14, y);
    y += 7;
    doc.text(`Total Sales Amount: ₹${totalSales.toFixed(2)}`, 14, y);
    y += 7;
    doc.text(`Total GST Collected: ₹${totalGST.toFixed(2)}`, 14, y);
    
    // Add footer
    doc.setFont(undefined, "normal");
    doc.setFontSize(8);
    doc.setTextColor(156, 163, 175);
    doc.text("This is an automatically generated report. For any queries, please contact support.", 105, 290, { align: "center" });
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === "salesOverview" && (
              ...
              >
                ...
                {/* 4. Remove the Sales by Category section from the Sales Overview
                        ...
                        >
                          ...
                          <div className="admin-recent-sale-details">
                            ...
                            <div className="admin-recent-sale-meta">
                              ...
                              <span className="admin-recent-sale-items">{invoice.items?.length || 0} items</span>
                            </div>
                          </div>
                          <div className="admin-recent-sale-amount">
                            <span className="admin-price">₹{invoice.totalBill}</span>
                            <span
                              className={`admin-badge ${invoice.paymentMethod === "cash" ? "admin-badge-success" : "admin-badge-info"}`}
                            >
            ...
            {/* 3. Replace the entire Transactions section with the updated UI */}
            {/* Transactions */}
            {activeTab === "transactions" && (
              ...
              >
                  ...
                  <div className="admin-header-actions">
                    <motion.button
                      ...
                      onClick={() => {
                        ...
                        script.onload = generateTransactionPDF;
                        document.body.appendChild(script);
                      }}
                    >
                      <Download className="admin-button-icon" /> Download PDF
                    </motion.button>
                    <motion.button
                      className="admin-button admin-button-outline"
                      whileHover={{ scale: 1.03 }}
                      whileTap={{ scale: 0.97 }}
                    >
                      <Printer className="admin-button-icon" /> Print
                    </motion.button>
                  </div>
                </header>
                ...
                <div className="admin-transaction-filters">
                  ...
                  <div className="admin-transaction-filter-options">
                    <div className="admin-filter-item">
                      <CalendarIcon className="admin-filter-icon" />
                    ...
                    <div className="admin-filter-item">
                      <ArrowDown className="admin-filter-icon" />
                      <select className="admin-filter-select">
                ...
                <div className="admin-transaction-summary">
                  <div className="admin-transaction-stat">
                    <div className="admin-transaction-stat-value">{invoices.length}</div>
                    <div className="admin-transaction-stat-label">Total Transactions</div>
                  </div>
                  
                  <div className="admin-transaction-stat">
                    <div className="admin-transaction-stat-value">₹{totalSales.toFixed(2)}</div>
                    <div className="admin-transaction-stat-label">Total Revenue</div>
                  </div>
                  
                  <div className="admin-transaction-stat">
                    <div className="admin-transaction-stat-value">₹{invoices.length > 0 ? (totalSales / invoices.length).toFixed(2) : "0.00"}</div>
                    <div className="admin-transaction-stat-label">Average Order Value</div>
                  </div>
                </div>

                <motion.div
                  className="admin-card admin-transaction-card"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.4, delay: 0.1 }}
                >
                  <div className="admin-table-container">
                    <table className="admin-data-table admin-transaction-table">
                      <thead>
                        <tr>
                          <th>Bill ID</th>
                          <th>Customer Name</th>
                        ...
                        {invoices
                          ...
                          .map((inv, index) => (
                            ...
                            >
                              ...
                              <td>
                                <span className={`admin-badge ${inv.paymentMethod === "cash" ? "admin-badge-success" : "admin-badge-info"}`}>
                              ...
                              <td>
                                <motion.button
                                  className="admin-button admin-button-small admin-button-view"
                                  onClick={() => setSelectedTransaction(inv)}
                                  whileHover={{ scale: 1.05 }}
                                  whileTap={{ scale: 0.95 }}
                                >
                                  <Eye className="admin-button-icon" size={14} /> View
                                </motion.button>
                              </td>
                            </motion.tr>
                          ))}
                      ...
                      >
                        ...
                        <div className="admin-modal-content">
                          <div className="admin-transaction-modal-header">
                            <div className="admin-transaction-modal-status">
                              <span className="admin-badge admin-badge-success">Completed</span>
                            </div>
                            <div className="admin-transaction-modal-actions">
                              <motion.button
                                className="admin-button admin-button-small admin-button-outline"
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                              >
                                <Printer className="admin-button-icon" size={14} /> Print Receipt
                              </motion.button>
                            </div>
                          </div>
                          ...
                          <div className="admin-customer-details">
                            ...
                            >
                              <p className="admin-detail-label">Transaction Date</p>
                              <p className="admin-detail-value">{new Date(selectedTransaction.date).toLocaleString()}</p>
                            </motion.div>
                          </div>
